...
if ($test || $preview) {
    // navigate to EmailManDelivery to send the queued emails immediately:
    $header_URL = "Location: index.php?action=EmailManDelivery&module=EmailMan&" .
        "campaign_id={$_REQUEST['record']}" .
        "&return_module={$return_module}&return_action={$return_action}&return_id={$return_id}&" .
        "mode={$_REQUEST['mode']}";
    if ($from_wiz) {
        $header_URL .= "&from_wiz=true";
    }
} else {
    //navigate back to campaign detail view, sending will be handled by cron job later:
