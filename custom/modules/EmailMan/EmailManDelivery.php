...
// previews are like tests for most things but different for others (target List selection, etc)
$preview = (isset($_REQUEST['mode']) && ($_REQUEST['mode'] === 'preview'));
$test = ((isset($_REQUEST['mode']) && ($_REQUEST['mode'] == 'test')) || $preview);
...
if ($test && !$preview) {
    ...
    $select_query .= ($confirmOptInEnabled ? ' OR related_confirm_opt_in = 1 ' : ' AND related_confirm_opt_in = 0');
...
do {
    ...
    while ($row = $db->fetchByAssoc($result)) {
        ...
        // copies all fields from an emailman db row into an object:
        ...
        if ((empty($row['related_confirm_opt_in']) || $row['related_confirm_opt_in'] == '0')) {
            if (!$emailman->sendEmail($mail, $massemailer_email_copy, $test, $preview)) {
...
if ($preview) {
    // calling with no content finishes off the preview file:
    $emailman->appendEmailToPreview();
    $header_URL = "Location: public/campaignPreview.html";
    header($header_URL);
}
elseif (isset($_REQUEST['return_module']) && isset($_REQUEST['return_action']) && isset($_REQUEST['return_id'])) {
    $from_wiz = ' ';
