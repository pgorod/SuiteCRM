...
(function ($) {
  ...
  $.fn.EmailsComposeView = function (options) {
    ...
    self.isBodyValid = function () {
      ...
      if ($(self).find('#description_html').val() !== '') { // was description
        return true;
      }
...
/*    self.tinyMceSetup = function (editor) {
      ...
      editor.on('change', function () {
        ...
        $(self).find('textarea#description_html').val(editor.getContent()); // was input#
...
*/
     ...
     * @returns {boolean}
     */
    self.onSendEmail = function (theEvent) {
      ...
      // differentiate Send button from Preview button:
      formData.append('preview', 'no');
      if (theEvent.target.className.includes('btn-preview')) {
      ...
      $.ajax({
        ...
        if (typeof response.errors !== "undefined") {
          mb.hideFooter();
          mb.showHeader();
          mb.setTitle('Server error');
          mb.setBody(response.errors.title);
          mb.showFooter();
          //$.fn.selectTextareaLine($('#description_html')[0], 4);
          $(self).trigger("sentEmailError", [self, response]);
        } else {

          if (response['preview-description_html']) {

            mb2.setTitle('Preview');
            mb2.controls.modal.buttons.ok[0].innerHTML = SUGAR.language.translate('Emails', 'LBL_SEND_BUTTON_LABEL');

            //var body = $('<input type="text" id="country" name="country" value="Norway" readonly></input><iframe id="preview-iframe" style="background-color: #FFF; width:100%; height:500px"/>')
            var body = '<input type="text" id="preview-subject" name="preview-subject" style="width:100%;margin-bottom:5px;" value="' +
                response['preview-name'] +
                '" readonly></input><iframe id="preview-iframe" style="background-color: #FFF; width:100%; height:500px"/>';
            var attach = '<div style="display:inline; margin-top: 15px"><div style="float:left"><b>Attachments:&nbsp;</b></div>' +
                '<div class="edit-view-field" style="float:left">' + response['preview-attachAsHTML'] +
                '</div></div><div style="clear:both"></div>';
            body=$(body)
                .on('load', function(){
                  $(this).contents().find('body').append(response['preview-description_html']);
                  $(this).closest('iframe').after(attach);
                  $(this).closest('.modal-dialog').css('width', '80%');
                  $('#preview-iframe').addClass('done');
                });

            //var body = '<iframe height="100%" scrolling="yes" frameborder="1">' + response.preview + '</iframe>';
            mb2.setBody(body);
            mb2.showFooter();
            mb.remove();
            mb2.show();
            mb2.on('ok', function () {
              "use strict";
              mb2.remove();
              $('.btn-send-email').find('[name=action]').val('send');
              $('.btn-send-email').submit();
            });

            mb2.on('cancel', function () {
              "use strict";
              mb2.remove();
            });
            //var myWindow = window.open("", "Preview", "width=200,height=100");
            //myWindow.document.write(response['preview']);
            return false;
          }
    ...
    self.attachFile = function (event) {
      ...
      var fileInput = $('<input>')
        ...
        .attr('multiple', 'true')
        .appendTo(fajaileGroupContainer);
...
*/
 ...
 //       opts.tinyMceOptions.selector = 'form[name="ComposeView"] textarea#description_html'; // was description
      }
        ...
        $.ajax({
          ...
          if ($(self).find('#is_only_plain_text').length === 1) {
            $(self).find('#is_only_plain_text').click(function () {
              var tinymceToolbar = $(tinymce.EditorManager.activeEditor.getContainer()).find('.mce-toolbar');
...
*/
      if (typeof tinymce === "undefined") {
        ...
        //$(self).find('[data-label="description"]').closest('.edit-view-row-item').addClass('hidden'); // was description_html
        // copy plain to html
        $(self).find('#description').closest('.edit-view-row-item').addClass('hidden'); // was description_html
        $(self).find('textarea#description_html').on("keyup", function () {
          $(self).find('textarea#description').val($(self).find('textarea#description_html').val()); //.replace('\n', '<br>'));
        });
      ...
      // clones Send button to create Preview button. When moved to Core, this should be in the tpl, not a JS hack
      var preview = $('.btn-send-email').clone().insertAfter($('.btn-send-email'));
      preview.attr('class', 'btn btn-preview').attr('title', 'Preview');
      preview.children(":first").removeClass('glyphicon-send').addClass('glyphicon-blackboard');
      preview.click(self.sendEmail);
  ...
  $.fn.EmailsComposeView.onTemplateSelect = function (args) {
    ...
    var confirmed = function (args) {
      ...
      $.post('index.php?entryPoint=emailTemplateData', {
        ...
        SuiteEditor.apply(response.data.body_from_html);
  ...
  $.fn.EmailsComposeView.defaults = {
    "tinyMceOptions": {
      ...
      menubar: false,
      toolbar: ['fontselect | fontsizeselect | bold italic underline | styleselect | code preview'],
      formats: {
        bold: {inline: 'b'},
        italic: {inline: 'i'},
        underline: {inline: 'u'}
